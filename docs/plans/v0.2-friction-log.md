...
| F38 | After PR #47 (NoDebt allowlist) shipped, T1.20 was eligible for retry but no automation re-attempted the transition — the gate fix was forward-only and the legacy code-kind evidence still triggers the marker scan | P2 | **fixed** (F49) | F49 ships `Durability::retry_task`, `POST /v1/tasks/:id/retry`, and `cvg task retry <id>`: failed → pending with a dedicated `task.retried` audit row, clearing the previous owner so any agent can re-claim. The "not retryable" wording in the `TaskStatus::Failed` doc was retired. Test coverage: 5 new tests in `crates/convergio-durability/tests/retry.rs`. The DELETE evidence endpoint mentioned in the original entry remains a separate item — retry is sufficient when the evidence corpus has been corrected upstream. |
